...
window.onload = function() {
   ...
   var bubbleImages = [];
var imageUrls = [
     "https://i.postimg.cc/jLcbhV6p/foto-A.jpg",
    "https://i.postimg.cc/t1KQg9yx/foto-G.jpg",
    "https://i.postimg.cc/FYKQWPD7/foto-iaia.jpg",
    "https://i.postimg.cc/XrD6Twzx/foto-iaio.jpg",
    "https://i.postimg.cc/3y4Q7kMh/foto-jordi.jpg",
    "https://i.postimg.cc/dkGMkgtS/foto-Ju.jpg",
    "https://i.postimg.cc/SY4BWVnQ/foto-L.jpg"
];

// bubble-shooter-example.js con las mejoras para los botones "Trayectoria", "Cambio Color" y "Derribo"

// Variable para mostrar la trayectoria
var showTrajectory = false;
// Variable para limitar el uso del cambio de color
var cambioColorUsed = false;
// Variable para limitar el uso del derribo
var derriboUsed = false;
...
function renderPlayer() {
    ...
    // Dibujar la trayectoria si está activada
    if (showTrajectory) {
        console.log("Dibujando Trayectoria...");
	context.setLineDash([5, 5]); // Líneas de puntos
        context.strokeStyle = "rgba(0, 0, 0, 1)";
        context.lineWidth = 4;
        context.beginPath();
        context.moveTo(centerx, centery);

        // Dibuja la trayectoria extendida
        var trajectoryX = centerx + 1000 * Math.cos(degToRad(player.angle));
        var trajectoryY = centery - 1000 * Math.sin(degToRad(player.angle));
        context.lineTo(trajectoryX, trajectoryY);
        context.stroke();
        context.setLineDash([]); // Restablecer a línea sólida
    }

    // Dibujar el lanzador y las burbujas
...
// Evento para el botón "Trayectoria"
document.getElementById("trayectoriaButton").addEventListener("click", () => {
    showTrajectory = true;
	console.log("Trayectoria Activada");

    // Desactiva la trayectoria después de un lanzamiento
    const originalShootBubble = shootBubble;
    shootBubble = function () {
        originalShootBubble();
        showTrajectory = false;
        shootBubble = originalShootBubble; // Restablece la función original
	console.log("Trayectoria Desactivada");
    };
});

// Evento para el botón "Cambio Color"
document.getElementById("cambioColorButton").addEventListener("click", () => {
    if (!cambioColorUsed) {
        cambioColorUsed = true;

        // Generar un color aleatorio distinto al actual
        const existingColors = findColors();
        let newColor;
        do {
            newColor = randRange(0, bubblecolors - 1);
        } while (newColor === player.nextbubble.tiletype);

        player.nextbubble.tiletype = newColor;
    }
});

// Función para derribar burbujas
function derribarBurbujas() {
    const midpoint = Math.floor(level.rows / 2);
    for (let i = 0; i < level.columns; i++) {
        for (let j = midpoint; j < level.rows; j++) {
            level.tiles[i][j].type = -1; // Vacía las burbujas
        }
    }
}

// Evento para el botón "Derribo"
document.getElementById("derriboButton").addEventListener("click", () => {
    if (!derriboUsed) {
        derriboUsed = true;
        derribarBurbujas();
    }
...
function loadBubbleImages(urls) {
...
document.addEventListener('DOMContentLoaded', () => {
    const backgroundAudio = new Audio('take a chance 160-DEF.mp3');
   ...
   function drawBubble(x, y, index) {
    ...
    var img = bubbleImages[index];
    if (img) {
        var imgSize = 20; // Tamaño de la imagen en el centro de la burbuja
        context.drawImage(img, x + (level.tilewidth - imgSize) / 2, y + (level.tileheight - imgSize) / 2, imgSize, imgSize);
    }
}
...
// Update the onMouseMove function to restrict the angle
